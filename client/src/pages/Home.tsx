  ...
  return (
    <div dir={isAr ? "rtl" : "ltr"} lang={language} className={`site-shell ${darkMode ? "night-mode" : ""}`}>
      ...
      <main id="top">
        <section className="hero-section">
          <div className="hero-copy reveal">
            ...
            <div className="hero-actions">
              ...
              <a className="text-link" href="https://maps.app.goo.gl/ghWyhYWDxdapX7zu6" target="_blank" rel="noreferrer"><MapPin size={16} /> {r("توجه إلينا", "Visit us")}</a>
            </div>
        ...
        <section id="location" className="location-section"><div className="location-copy"><div className="eyebrow"><span className="eyebrow-line" /> {r("موقعنا", "Find us")}</div><h2>{r("توجه إلينا", "Come see us")}.<br /><em>{r("ننتظرك.", "We are ready.")}</em></h2><p>{r("نحن في موقع واضح وسهل الوصول على طريق B Ring Road في الدوحة.", "Find us on B Ring Road in Doha, with easy access and clear directions.")}</p><a className="primary-button" href="https://maps.app.goo.gl/ghWyhYWDxdapX7zu6" target="_blank" rel="noreferrer"><MapPin size={16} /> {r("افتح الخريطة", "Open map")}</a></div><div className="map-wrap"><div className="map-frame"><iframe title={r("خريطة موقع Triple Star Car Wash", "Triple Star Car Wash location map")} src="https://www.google.com/maps?q=25.2786128,51.5422532&z=17&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div><div className="map-place-label"><div><strong>{r("تربل ستار لغسيل السيارات", "Triple Star Car Wash")}</strong><span>{r("طريق B Ring Road، الدوحة", "B Ring Road, Doha")}</span></div><a href="https://maps.app.goo.gl/ghWyhYWDxdapX7zu6" target="_blank" rel="noreferrer">{r("توجه إلينا", "Visit us")} <MapPin size={14} /></a></div></div></section>

        <section id="contact" className="contact-section"><div className="contact-content"><div className="eyebrow light"><span className="eyebrow-line" /> {r("جاهز للمعان القادم؟", "Ready for the next shine?")}</div><h2>{r("اترك الباقي", "Leave the rest")}<br /><em>{r("علينا.", "to us.")}</em></h2><p>{r("اتصل بنا أو احصل على الاتجاهات الآن.", "Call us or get directions now.")}</p><div className="contact-actions"><a className="gold-button" href="tel:+97433022544"><Phone size={16} /> +974 3302 2544</a><a className="outline-button" href="https://maps.app.goo.gl/ghWyhYWDxdapX7zu6" target="_blank" rel="noreferrer"><MapPin size={16} /> B Ring Rd، Doha</a></div></div><div className="contact-mark"><img src={MARK_IMAGE} alt="Triple Star Car Wash" /><span>TRIPLE STAR<br />CAR WASH</span></div></section>
      </main>

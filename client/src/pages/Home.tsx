  ...
  return (
    <div dir={isAr ? "rtl" : "ltr"} lang={language} className={`site-shell ${darkMode ? "night-mode" : ""}`}>
      ...
      <main id="top">
        ...
        <section id="location" className="location-section"><div className="location-copy"><div className="eyebrow"><span className="eyebrow-line" /> {r("موقعنا", "Find us")}</div><h2>{r("توجه إلينا", "Come see us")}.<br /><em>{r("ننتظرك.", "We are ready.")}</em></h2><p>{r("نحن في موقع واضح وسهل الوصول على طريق B Ring Road في الدوحة.", "Find us on B Ring Road in Doha, with easy access and clear directions.")}</p><a className="primary-button" href="https://maps.google.com/?q=25.2786128,51.5422532" target="_blank" rel="noreferrer"><MapPin size={16} /> {r("افتح الخريطة", "Open map")}</a></div><div className="map-wrap"><div className="map-frame"><iframe title={r("خريطة موقع Triple Star Car Wash", "Triple Star Car Wash location map")} src="https://www.google.com/maps?q=25.2786128,51.5422532&z=16&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div><div className="map-place-label"><div><strong>{r("تربل ستار لغسيل السيارات", "Triple Star Car Wash")}</strong><span>{r("طريق B Ring Road، الدوحة", "B Ring Road, Doha")}</span></div><a href="https://maps.google.com/?q=25.2786128,51.5422532" target="_blank" rel="noreferrer">{r("توجه إلينا", "Visit us")} <MapPin size={14} /></a></div></div></section>
